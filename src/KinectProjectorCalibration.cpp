...
KinectProjectorCalibration::KinectProjectorCalibration() {	
	...
	fastCheckResize = 1.0;
	chessboardColor = 125;
	kinectColorImage.allocate(512,424);
	stableFrom = -1;
}
...
void KinectProjectorCalibration::resetTimer() {
	stableFrom = -1;
}
...
//	colorImg.resize(colorImg.width*fastCheckResize,colorImg.height * fastCheckResize);
	...
	if(b) {
		...
		for (int i = 0; i < pointBuf.size(); i++) {
			pointBufFastCheck.push_back(toOf(pointBuf[i]));
		}
		//cout << "Stable: " << (ofGetElapsedTimeMillis() - stableFrom) << endl;
		return (ofGetElapsedTimeMillis() - stableFrom) ;
	} else {
		stableFrom = -1;
		return stableFrom;
	}
...
bool	KinectProjectorCalibration::addCurrentFrame(){
...
void KinectProjectorCalibration::setOffsets(int x, int y) {
	offsetX = x;
	offsetY = y;
}

int KinectProjectorCalibration::getOffsetX() {
	return offsetX;
}
int KinectProjectorCalibration::getOffsetY() {
	return offsetY;
}
...
bool KinectProjectorCalibration::clean(float minReprojectionError) {
	...
	if(worldCoordinatesChessboardBuffer.size() > 0) {
		if(removed > 0) {
			cout << "Points removed - recalibrating" << endl; 
			return calibrate();
		} else {
			cout << "No points removed" << endl;
			return true;
		}
	} else {
		reprojError = 0;
...
void	KinectProjectorCalibration::drawChessboardDebug(float x, float y, float width, float height)
{
	...
	chessboard.setPatternSize(chessboardSize/100.0);
...
void	KinectProjectorCalibration::drawReprojectedPointsDebug(float x, float y, float width, float height){
	...
	for (int i = worldCoordinatesChessboardBuffer.size() - 1; i >= 0 ; i--) {
		...
		for (int j = 0; j < points.size(); j++) {
			ofSetColor(255,0,0);
			ofCircle(imageCoordinatesChessboardBuffer[i][j].x, imageCoordinatesChessboardBuffer[i][j].y, 6);
			ofSetColor(0,255,0);
			ofCircle(points[j].x, points[j].y, 8);
			times++;
		}
		if (times > 5) break; //for performance reasons		
	}
	ofPopMatrix();
	ofPopStyle();
}
...
void	KinectProjectorCalibration::drawProcessedInputDebug(float x, float y, float width, float height)
{
	...
	if(chessboardFound && kinectCoordinatesChessboardBuffer.size() > 0) {
		...
		for (int i = 0; i < kinectCoordinatesChessboardBuffer[lastElement].size(); i++) {
			ofCircle(kinectCoordinatesChessboardBuffer[lastElement][i].x,kinectCoordinatesChessboardBuffer[lastElement][i].y,5);
		}
	}		
	if (stableFrom != -1)
		ofDrawBitmapStringHighlight(ofToString((int)(2500 - ofGetElapsedTimeMillis() + stableFrom)) + " ms to capture...", 20,20);
	ofPopStyle();
	ofPopMatrix();
	ofSetColor(255,255,255,255);
}